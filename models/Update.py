...
class LocalUpdate_KD(object):
    def __init__(self, args, net_glob, dataset=None, idxs=None, verbose=False):
        self.args = args
        self.loss_func = nn.CrossEntropyLoss()
        self.ensemble_loss = nn.KLDivLoss(reduction="batchmean")
        self.selected_clients = []
        self.ldr_train = DataLoader(DatasetSplit(dataset, idxs), batch_size=self.args.local_bs, shuffle=True)
        self.glob_model = net_glob
        self.prox_alpha = args.prox_alpha
        self.verbose = verbose

    def train(self, alpha, net, T):
        ...
        global_weight_collector = list(copy.deepcopy(self.glob_model).parameters())
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                kl_loss = KD(log_probs, teacher(images)['output'], T)
                loss = predictive_loss + fed_prox_reg + kl_loss
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                for previous_net in self.old_models:
                    previous_net.to(self.args.device)
...
class LocalUpdate_FedASync(object):
    def __init__(self, args, glob_model, dataset=None, idxs=None, verbose=False):
        self.args = args
        self.loss_func = nn.CrossEntropyLoss()
        self.ensemble_loss = nn.KLDivLoss(reduction="batchmean")
...
class LocalUpdate_WKAFL(object):
    def __init__(self, args, dataset=None, idxs=None, verbose=False):
        self.args = args
        self.loss_func = nn.CrossEntropyLoss()
        self.selected_clients = []
    ...
    def train(self, round, net):
        ...
        delta = []
        with torch.no_grad():
            for para in model_copy.parameters():
                delta.append(para)
            for grad_idx, params in enumerate(net.parameters()):

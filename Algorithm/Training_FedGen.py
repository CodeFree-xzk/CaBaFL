...
def FedGen(args, net_glob, dataset_train, dataset_test, dict_users):
    ...
    generative_model = Generator(args.dataset, embedding=False, latent_layer_idx=-1)
    ...
    for iter in range(args.epochs):
        ...
        for idx in idxs_users:
            local = LocalUpdate_FedGen(args=args, generative_model=generative_model, dataset=dataset_train,
                                       idxs=dict_users[idx], regularization=iter != 0)
            user_model = local.train(round=iter, net=copy.deepcopy(net_glob).to(args.device))
...
def train_generator(args, net_glob, generative_model, models, users, label_counts, batch_size, epoches=1,
                    latent_layer_idx=-1, verbose=False):
    """
    ...
    generative_optimizer = torch.optim.Adam(params=generative_model.parameters(), lr=args.ensemble_lr,
